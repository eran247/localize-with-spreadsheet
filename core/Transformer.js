...
const androidTransformer = {
  ...
  transformKeyValue: function(key, value) {
    ...
    let transformedKey = key.toLowerCase().replace(' ', '_');
  
    // Count the number of '%@' placeholders in the value
    const placeholderCount = (normalizedValue.match(/%@/g) || []).length;
  
    // Generate numbered placeholders using '%1$s', '%2$s', '%3$s', etc.
    const numberedPlaceholders = Array.from(Array(placeholderCount), (_, index) => `%${index + 1}$s`);
  
    // Replace the '%s' placeholders with the numbered placeholders
    for (let i = 0; i < numberedPlaceholders.length; i++) {
      normalizedValue = normalizedValue.replace(/%@/, numberedPlaceholders[i]);
    }
  
    const output = `  <string name="${transformedKey}">${normalizedValue}</string>`;
    return output;
  }, 
